...
function Dropdown({options, currentOption, onChange}) {
  return (
    <Menu as="div" className="relative inline-block text-left ">
      ...
      >
        <Menu.Items className="absolute left-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-gray-600 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
           {options.map(option => <Menu.Item key={option}>
              {({ active }) => (
                <span
                ...
                >
                  {currentOption === option ? <CheckIcon className="-mr-1 h-5 w-5 text-gray-300" aria-hidden="true" /> : ''}
                  {option}
                </span>
              )}
            </Menu.Item>)}
