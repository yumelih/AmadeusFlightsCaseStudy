...
function DatePick({ id, placeholder}) {
  ...
  return (
    <div className="relative">
      <div className="relative z-50 mt-2 rounded-md shadow-sm">
        <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
            <CalendarDaysIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
        </div>
        <DatePicker className="relative z-50 block w-full rounded-md border-0 py-1.5 pl-10 text-gray-100 ring-2 ring-inset ring-gray-500 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-gray-600 sm:text-sm sm:leading-6 bg-transparent" selected={id === 'departureDate' ? departureDate : arrivalDate} onChange={(date) => id === "departureDate" ? handleDepatureDate(date) : handleArrivalDate(date)} placeholderText={placeholder}/>
      </div>
    </div>
  );
};
